...
ggplot(resdata,aes(x=log2FoldChange,y=-log10(padj))) +
  ...
  geom_text(aes(label=c(label), color = -log10(padj)), size = 3, vjust = 1.5, hjust = 1, position = position_jitterdodge(jitter.width = 0.3, dodge.width = 0.3))+
...
condition_Low_vs_Full_0.001 <-
  subset(result_ordered_try , padj < 0.001)
nrow(condition_Low_vs_Full_0.001) #2320
head(condition_Low_vs_Full_0.001)


allsig <-
  merge(counts_table_short, condition_Low_vs_Full_0.001, by = 0)
sigcount <- allsig[, 2:8]
head(sigcount)
